...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements
    ...
    private void notifyWearables() {
        ...
        String highTemperature = Utility.formatTemperature(getContext(), cursor.getDouble(INDEX_MAX_TEMP));
        String lowTemperature = Utility.formatTemperature(getContext(), cursor.getDouble(INDEX_MIN_TEMP));
        int iconId = Utility.getIconResourceForWeatherCondition(weatherId);

        Bitmap iconBitmap = BitmapFactory.decodeResource(getContext().getResources(), iconId);

        PutDataMapRequest dataMapRequest = PutDataMapRequest.create("/sunshine-weather-change");
        dataMapRequest.getDataMap().putString("high-temperature", highTemperature);
        dataMapRequest.getDataMap().putString("low-temperature", lowTemperature);
        dataMapRequest.getDataMap().putAsset("icon", createAssetFromBitmap(iconBitmap));

        PutDataRequest dataRequest = dataMapRequest.asPutDataRequest();
        Wearable.DataApi.putDataItem(mGoogleApiClient, dataRequest).setResultCallback(new ResultCallbacks<DataApi.DataItemResult>() {
            @Override
